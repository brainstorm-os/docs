...
Au premier lancement, Brainstorm demande où votre savoir doit vivre.

1. Cliquez sur **Créer un coffre**.
2. Choisissez un dossier vide sur votre disque.
3. Décidez comment le protéger :
   - **Trousseau du système** (recommandé) — Brainstorm stocke la clé dans le trousseau de votre OS ; le coffre se déverrouille automatiquement lorsque vous vous connectez.
...
Brainstorm s'ouvre sur un tableau de bord. Tout ce que vous faites se passe à l'intérieur d'une **application**.
...
- Commencez à taper pour créer une note.
- Utilisez le texte enrichi — titres, listes, cases à cocher, code.
- Tapez `@` pour créer un lien vers un autre objet de votre coffre, ou `/` pour un menu de commandes.

Tout ce que vous écrivez est enregistré dans votre coffre sur le disque, immédiatement et hors ligne.

## 4. Relier votre savoir

La valeur de Brainstorm vient des relations que les objets entretiennent entre eux :

- Ouvrez **Base de données** pour créer un tableau structuré — tâches, contacts, liste de lecture — avec des propriétés typées.
